...
const getRedirectPath = (role) => {
  ...
  if (r === 'operateur' || r === 'opérateur')    return '/dashboard/Dashboard_operateur';
  return '/dashboard'; 
};
...
export default function AuthInterface({ onBack }) {
  ...
  // ── Utilitaire : extraire le message d'erreur du backend ──
  const getErrorMsg = (error, fallback) =>
    error.response?.data?.message || error.response?.data?.error || fallback;

  // ── FIX : sauvegarde avec nom_complet (même clé que le backend) ──
  const saveUserToStorage = (user) => {
    console.log('SAVING USER:', user); // ← ajoute cette ligne
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage(getErrorMsg(error, "E-mail ou mot de passe incorrect."));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage(getErrorMsg(error, "Erreur lors de l'inscription. Veuillez réessayer."));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      setErrorMessage(getErrorMsg(error, "Erreur lors de la connexion Google."));
    } finally {
      setIsLoading(false);
    }
  };

  const handleGoogleError = () => setErrorMessage("Échec de l'authentification Google.");
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center font-sans antialiased overflow-x-hidden select-none py-8 lg:py-0">
      <div className="absolute inset-0 z-0 overflow-hidden">
        <video autoPlay loop muted playsInline preload="none" className="w-full h-full object-cover" src="/video/video01.mp4" />
                ...
                <motion.div key="login" variants={formVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                  ...
                  <form onSubmit={handleLoginSubmit} className="space-y-4">
                    ...
                    <div className="relative w-full mt-4">
                      <div className="w-full flex items-center justify-center gap-3 border border-slate-300 text-slate-600 font-medium py-3 rounded-full hover:bg-slate-50 transition-all text-sm shadow-sm pointer-events-none select-none">
